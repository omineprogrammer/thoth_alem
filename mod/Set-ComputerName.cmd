...
:Main
	...
	call:Get-WMICasCSV %tmp_file% "%kronos%" NETUse "Get LocalName,Persistent,RemotePath,RemotePath,Status,UserName"
	type "%tmp_file%">>"%out_file%"
	...
	call:Get-WMICasCSV %tmp_file% "%kronos%" OS "Get BuildNumber,Caption,InstallDate,LastBootUpTime,LastBootUpTime,LastBootUpTime,NumberOfUsers,OSArchitecture,SerialNumber,Version"
	type "%tmp_file%">>"%out_file%"
	
	call:Get-WMICasCSV %tmp_file% "%kronos%" Partition "Get Bootable,DeviceID,Size,StartingOffset,Type"
	type "%tmp_file%">>"%out_file%"
	
	call:Get-WMICasCSV %tmp_file% "%kronos%" Printer "Get DeviceID,DriverName,Location,PortName,PrintJobDataType,Shared,ShareName,ShareName"
	type "%tmp_file%">>"%out_file%"
	...
	call:Get-WMICasCSV %tmp_file% "%kronos%" Volume "Get BlockSize,Capacity,Compressed,DeviceID,DeviceID,FileSystem,FreeSpace,Label,SerialNumber,SystemVolume"
	type "%tmp_file%">>"%out_file%"
